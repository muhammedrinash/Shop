...
const SingleProduct = ({ addToCart }) => {
  ...
  useEffect(() => {
    ...
    API.get(`/products/${id}`)
      ...
      .then((res) => {
        // Filter out the current product and pick up to 4 suggestions
        const otherProducts = res.data.filter((p) => p._id !== id);
        // We can just shuffle or take the first 4 for suggestions
  ...
  if (loading) {
    return <div className="pt-32 pb-20 text-center text-white min-h-screen">Loading...</div>;
  }

  if (!product) {
    return <div className="pt-32 pb-20 text-center text-white min-h-screen">Product not found.</div>;
  }

  return (
    <div className="pt-32 pb-20 px-6 max-w-[1440px] mx-auto min-h-screen text-white">
      <Link to="/store" className="inline-flex items-center gap-2 text-zinc-400 hover:text-white transition-colors mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="flex flex-col justify-center">
          ...
          <h1 className="text-4xl md:text-6xl font-black italic tracking-tighter mb-6">{product.name}</h1>
          ...
          <div className="mb-10 text-zinc-400 leading-relaxed text-lg">
            {product.description || "Experience the pinnacle of design and functionality. This premium item is crafted with meticulous attention to detail, ensuring unparalleled quality and performance that elevates your everyday lifestyle."}
          </div>

          <div className="mb-10">
            <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-500 mb-4">Stock Status</h3>
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${product.stock > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
              <span className="font-medium">{product.stock > 0 ? `${product.stock} Available in Stock` : 'Out of Stock'}</span>
            </div>
          </div>
      ...
      {suggestions.length > 0 && (
        <div className="mt-32">
          <h3 className="text-3xl font-bold tracking-tight italic mb-10">You Might Also Like</h3>
